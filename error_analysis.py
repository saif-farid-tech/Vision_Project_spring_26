"""
error_analysis.py
Detailed error analysis for SHViT-S4 on the SUN397 test set
...
Usage:
    python error_analysis.py \\
        --results-dir CV_Research_Paper_SUN397/Stage\\ 4:\\ Benchmarking\\ and\\ Demo/analysis/results \\
        --data-root   data \\
        --shvit-dir   SHViT \\
        --checkpoint  "CV_Research_Paper_SUN397/Stage 3: fine-tuning SHViT/shvit_s4/best.pth" \\
        --output-dir  CV_Research_Paper_SUN397/Stage\\ 4:\\ Benchmarking\\ and\\ Demo/analysis/error_analysis_outputs
"""
...
NUM_CLASSES = 397  # SUN397 / CoOp split
N_WORST_HEATMAP = 20
N_WORST_BARS = 15
N_GRID_ROWS = 4
N_GRID_COLS = 4
...
def plot_worst_15(per_class_acc, class_names, out_path: Path):
    ...
    ax.set_title(f"{N_WORST_BARS} worst-performing classes (SHViT-S4 on SUN397)")
...
def plot_misclassified_grid(args, all_preds, all_targets, class_names, device, test_ds, out_path: Path):
    ...
    fig.suptitle("Random misclassified test images (SHViT-S4, SUN397)", fontsize=12)
    fig.tight_layout()
    fig.savefig(out_path, dpi=300, facecolor="white", bbox_inches="tight")
    plt.close(fig)
    print(f"Wrote {out_path}")
...
def write_summary(results, class_names, top_pairs, test_size, out_path: Path):
    ...
    lines = [
        "SHViT-S4 Error Analysis Summary (SUN397)",
        "=" * 48,
        "",
        "Overall:",
...
def parse_args():
    ...
    p.add_argument("--results-dir", type=Path,
                   default=Path("CV_Research_Paper_SUN397/Stage 4: Benchmarking and Demo/analysis/results"))
    p.add_argument("--data-root",   type=Path, default=Path("data"))
    p.add_argument("--shvit-dir",   type=Path, default=_REPO_ROOT / "SHViT")
    p.add_argument("--checkpoint",  type=Path,
                   default=Path("CV_Research_Paper_SUN397/Stage 3: fine-tuning SHViT/shvit_s4/best.pth"))
    p.add_argument("--output-dir",  type=Path,
                   default=Path("CV_Research_Paper_SUN397/Stage 4: Benchmarking and Demo/analysis/error_analysis_outputs"))
...
def main():
    ...
    # ---- Class names from SUN397 test split ------------------------
