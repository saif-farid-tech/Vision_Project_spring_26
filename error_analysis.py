"""
error_analysis.py
Detailed error analysis for SHViT-S4 on the FGVC Aircraft test set
...
Usage:
    python error_analysis.py \\
        ...
        --output-dir  CV_Research_Paper_FGVCAircraft/Stage\\ 4:\\ Benchmarking\\ and\\ Demo/analysis/error_analysis_outputs
"""
...
NUM_CLASSES = 100  # FGVC Aircraft / CoOp split
N_WORST_HEATMAP = 20
N_WORST_BARS = 15
N_GRID_ROWS = 4
N_GRID_COLS = 4
...
def plot_worst_15(per_class_acc, class_names, out_path: Path):
    ...
    ax.set_title(f"{N_WORST_BARS} worst-performing classes (SHViT-S4 on FGVC Aircraft)")
...
def plot_misclassified_grid(args, all_preds, all_targets, class_names, device, test_ds, out_path: Path):
    ...
    fig.suptitle("Random misclassified test images (SHViT-S4, FGVC Aircraft)", fontsize=12)
    fig.tight_layout()
    fig.savefig(out_path, dpi=300, facecolor="white", bbox_inches="tight")
    plt.close(fig)
    print(f"Wrote {out_path}")
...
def write_summary(results, class_names, top_pairs, test_size, out_path: Path):
    ...
    lines = [
        "SHViT-S4 Error Analysis Summary (FGVC Aircraft)",
        "=" * 48,
        "",
        "Overall:",
...
def parse_args():
    ...
    p.add_argument("--checkpoint",  type=Path,
                   default=Path("CV_Research_Paper_FGVCAircraft/Stage 3: fine-tuning SHViT/shvit_s4/best.pth"))
    p.add_argument("--output-dir",  type=Path,
                   default=Path("CV_Research_Paper_FGVCAircraft/Stage 4: Benchmarking and Demo/analysis/error_analysis_outputs"))
...
def main():
    ...
    # ---- Class names from FGVC Aircraft test split ------------------------
