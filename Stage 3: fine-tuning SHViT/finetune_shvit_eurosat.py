"""
finetune_shvit_eurosat.py

Single-GPU wrapper for fine-tuning SHViT (S1..S4) on EuroSAT.
...
Usage:
    python finetune_shvit_eurosat.py \\
...
def get_args():
    p = argparse.ArgumentParser("SHViT EuroSAT fine-tuning (single GPU)")
    ...
    p.add_argument("--output-dir",  type=Path,
                   default=Path("CV_Research_Paper_EuroSAT/Stage 3: fine-tuning SHViT/shvit_s4"))
...
def build_loaders(args):
    ...
    print(f"EuroSAT split: {len(train_ds):,} train  {len(val_ds):,} val")
