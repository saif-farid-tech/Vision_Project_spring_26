"""
finetune_shvit_oxford_pets.py

Single-GPU wrapper for fine-tuning SHViT (S1..S4) on Oxford Pets.
...
Usage:
    python finetune_shvit_oxford_pets.py \\
...
def get_args():
    p = argparse.ArgumentParser("SHViT Oxford Pets fine-tuning (single GPU)")
    ...
    p.add_argument("--output-dir",  type=Path,
                   default=Path("CV_Research_Paper_OxfordPets/Stage 3: fine-tuning SHViT/shvit_s4"))
...
def build_loaders(args):
    ...
    print(f"Oxford Pets split: {len(train_ds):,} train  {len(val_ds):,} val")
