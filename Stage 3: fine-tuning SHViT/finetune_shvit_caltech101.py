"""
finetune_shvit_caltech101.py

Single-GPU wrapper for fine-tuning SHViT (S1..S4) on Caltech-101.
...
Usage:
    python finetune_shvit_caltech101.py \\
...
def get_args():
    p = argparse.ArgumentParser("SHViT Caltech-101 fine-tuning (single GPU)")
    ...
    p.add_argument("--output-dir",  type=Path,
                   default=Path("CV_Research_Paper_Caltech101/Stage 3: fine-tuning SHViT/shvit_s4"))
...
def build_loaders(args):
    ...
    print(f"Caltech-101 split: {len(train_ds):,} train  {len(val_ds):,} val")
