# Oxford Pets Classification with SHViT

COE-486 Computer Vision — AUS Spring 2026.
Oxford Pets variant of the SHViT vs. baselines experiment. Data preprocessing
...
but using the Oxford-IIIT Pet dataset (37 cat and dog breeds from Parkhi et
al., 2012). Every notebook writes its outputs under the root directory
**`CV_Research_Paper_OxfordPets/`** so the final artifacts mirror the stage
layout one-to-one.
...
We fine-tune all four SHViT variants (S1–S4) on Oxford Pets from
...
- Split file: `<data-root>/oxford_pets/split_zhou_OxfordPets.json`
...
- Image directory: `<data-root>/oxford_pets/images/<Breed>_<id>.jpg`
  (Oxford Pets ships every image in one flat folder; breed is encoded in
  the filename). The Tip-Adapter style split JSON stores image paths
  relative to `<data-root>/oxford_pets/images/`.
- Annotations: `<data-root>/oxford_pets/annotations/{trainval.txt, test.txt}`,
  used by the fallback split generator. `torchvision.datasets.OxfordIIITPet`
  downloads and extracts everything automatically; the prep script
  re-shapes the unpack into the Tip-Adapter style directory layout.
...
`CV_Research_Paper_OxfordPets/`** so the final layout looks like:
...
**Root**
- `README.md` — this file.
- `prepare_oxford_pets.py` — downloads Oxford Pets and prepares the Tip-Adapter
  style split JSON (gdown-first, deterministic-fallback).
- `splits.py` — loads the Tip-Adapter Oxford Pets split into a
...
- `eda.py` — Oxford Pets EDA visualizations (sample grid, image sizes,
...
Contains the `Datum`, `DatasetBase`, `DatasetWrapper`, and `OxfordPets`
helpers, plus the `auto_prepare` extension used by `prepare_oxford_pets.py`.

**Stage 1 — `Stage 1: testing the SHViT model/`**
- `setup_shvit.sh` — clones SHViT and installs its dependencies.
- `prepare_oxford_pets.py` — thin wrapper that calls the root prepare script.
- `verify_model.py` — sanity-check SHViT-S4 on a handful of Oxford Pets images.
- `SHViT_OxfordPets_Colab.ipynb` — Colab notebook for the stage.

**Stage 2 — `Stage 2: baseline models/`**
- `train_baseline.py` — fine-tunes ResNet-50 or MobileNetV2 on Oxford Pets.
- `Baselines_OxfordPets_Colab.ipynb` — Colab notebook for the stage.

**Stage 3 — `Stage 3: fine-tuning SHViT/`**
- `finetune_shvit_oxford_pets.py` — fine-tunes any SHViT variant (S1..S4)
  on Oxford Pets with the paper's recipe.
- `SHViT_Finetune_OxfordPets_Colab.ipynb` — Colab notebook for the stage.

**Stage 4 — `Stage 4: Benchmarking and Demo/`**
- `Analysis_Pipeline_OxfordPets_Colab.ipynb` — ties `evaluate_all.py`,
...
```bash
# 1. Prepare data
python prepare_oxford_pets.py --root data

# 2. Train baselines
python "Stage 2: baseline models/train_baseline.py" \
    --model resnet50 --data-root data \
    --output-dir "CV_Research_Paper_OxfordPets/Stage 2: baseline models"
python "Stage 2: baseline models/train_baseline.py" \
    --model mobilenet_v2 --data-root data \
    --output-dir "CV_Research_Paper_OxfordPets/Stage 2: baseline models"

# 3. Fine-tune SHViT variants (S1..S4) — requires SHViT repo + weights
python "Stage 3: fine-tuning SHViT/finetune_shvit_oxford_pets.py" \
    --model shvit_s4 --shvit-dir SHViT --finetune weights/shvit_s4.pth \
    --data-root data \
    --output-dir "CV_Research_Paper_OxfordPets/Stage 3: fine-tuning SHViT/shvit_s4"
