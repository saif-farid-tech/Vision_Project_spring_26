...
Usage:
    python make_figures.py \\
        ...
        --logs-dir    CV_Research_Paper_SUN397 \\
        --output-dir  CV_Research_Paper_SUN397/Stage\\ 4:\\ Benchmarking\\ and\\ Demo/analysis/figures
"""
...
def fig_training_curves(rows, out_path: Path):
    ...
    ax.set_title("Validation top-1 over training (SUN397)")
...
def fig_train_loss(rows, out_path: Path):
    ...
    ax.set_title("Training loss over epochs (SUN397)")
    ax.grid(True, alpha=0.3)
    ax.legend(loc="center left", bbox_to_anchor=(1.02, 0.5),
              frameon=True, title="Model")
    _save(fig, out_path)
...
def fig_speed_vs_accuracy(rows, out_path: Path):
    ...
    ax.set_title("Speed vs. accuracy on SUN397 test set")
    ax.grid(True, alpha=0.3)
    _save(fig, out_path)
...
def fig_accuracy_bars(rows, out_path: Path):
    ...
    ax.set_ylabel("Test top-1 accuracy (%)")
    ax.set_title("Test top-1 accuracy on SUN397 (sorted)")
...
def main():
    ...
    p.add_argument("--logs-dir",    type=Path,
                   default=Path("CV_Research_Paper_SUN397"),
                   help="Output root containing the Stage 2 / Stage 3 log subdirectories")
    p.add_argument("--output-dir",  type=Path,
                   default=Path("CV_Research_Paper_SUN397/Stage 4: Benchmarking and Demo/analysis/figures"))
    args = p.parse_args()
