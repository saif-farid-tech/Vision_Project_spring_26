"""tip_datasets/utils.py

Verbatim port of Tip-Adapter's data-pipeline utilities:
    https://github.com/gaopengcuhk/Tip-Adapter/blob/main/datasets/utils.py

This is the data preprocessing + data-handling layer used by every notebook
and script in this branch (the ``_Food101`` variant). It provides:

  * ``Datum``                 — a single (impath, label, classname) record.
  * ``DatasetBase``           — the unified dataset container (train/val/test
                                splits, few-shot sampling, label bookkeeping).
  * ``DatasetWrapper``        — a ``torch.utils.data.Dataset`` that reads an
                                image from disk and applies the transform.
                                Its built-in ``to_tensor`` pipeline uses
                                **BICUBIC resize + ToTensor + CLIP normalization**
                                (mean/std below) — i.e. CLIP's preprocessing,
                                not ImageNet's.
  * ``build_data_loader``     — convenience DataLoader factory.

Only cosmetic header docs differ from the upstream file; the class/function
bodies are identical so the data preprocessing matches Tip-Adapter exactly.
"""
