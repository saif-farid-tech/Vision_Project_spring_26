"""Vendored from https://github.com/gaopengcuhk/Tip-Adapter/blob/main/datasets/utils.py

The only change vs. upstream: `gdown` is imported lazily inside DatasetBase.download_data
so importing this module does not require gdown to be installed.
"""
...
class DatasetBase:
    ...
    def download_data(self, url, dst, from_gdrive=True):
        ...
        if from_gdrive:
            import gdown  # lazy import — only required when actually downloading
...
class DatasetWrapper(TorchDataset):
    """Tip-Adapter style wrapper that emits (image_tensor, label).

    Default normalization is the CLIP / Tip-Adapter normalization
    (mean=(0.48145466, 0.4578275, 0.40821073),
     std =(0.26862954, 0.26130258, 0.27577711)).

    Bicubic interpolation is used for the no-augmentation `to_tensor` path.
    """
