"""
eda.py
UCF101 EDA visualizations for the report's data section.
...
Usage:
    python eda.py --data-root data \\
        --output-dir CV_Research_Paper_UCF101/Stage\\ 4:\\ Benchmarking\\ and\\ Demo/analysis/eda_outputs
"""
...
def load_datasets(data_root: Path):
    ...
    ds = UCF101(root=str(data_root), num_shots=-1)
...
def plot_sample_grid(items, classes, out_path: Path, n_rows=5, n_cols=5):
    ...
    for ax, idx in zip(axes.flat, indices):
        ...
        ax.set_xticks([])
        ax.set_yticks([])
    fig.suptitle("UCF101 random training samples", fontsize=12)
    fig.tight_layout()
...
def plot_class_distribution(train, test, classes, out_path: Path):
    ...
    ax.set_title("UCF101 class distribution (alphabetical)")
    ax.legend()
    fig.tight_layout()
    fig.savefig(out_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    p.add_argument(
        "--output-dir",
        type=Path,
        default=Path("CV_Research_Paper_UCF101/Stage 4: Benchmarking and Demo/analysis/eda_outputs"),
    )
